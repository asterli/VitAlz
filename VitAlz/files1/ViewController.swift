...
class ViewController: UIViewController, AVAudioPlayerDelegate {
    ...
    func setupAudioFiles (){
        let soundFilePath = Bundle.main.url(forResource: "1", withExtension: "wav")
        let soundFileURL = soundFilePath
        
        let soundFilePath2 = Bundle.main.url(forResource: "2", withExtension: "wav")
        let soundFileURL2 = soundFilePath2
        
        let soundFilePath3 = Bundle.main.url(forResource: "3", withExtension: "wav")
        let soundFileURL3 = soundFilePath3
        
        let soundFilePath4 = Bundle.main.url(forResource: "4", withExtension: "wav")
        let soundFileURL4 = soundFilePath4
        
        if let url = soundFileURL,
           let url2 = soundFileURL2,
           let url3 = soundFileURL3,
           let url4 = soundFileURL4 {
            do {
                sound1Player = try AVAudioPlayer(contentsOf: url)
                sound2Player = try AVAudioPlayer(contentsOf: url2)
                sound3Player = try AVAudioPlayer(contentsOf: url3)
                sound4Player = try AVAudioPlayer(contentsOf: url4)
            } catch {
                print("Error initializing AVAudioPlayer: \(error.localizedDescription)")
            }
        } else {
            print("Error: One or more soundFileURLs are nil")
        }
